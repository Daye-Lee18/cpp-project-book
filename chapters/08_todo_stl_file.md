...
이번 장의 핵심은 프로그램이 종료되어도 데이터를 남기는 방법입니다. `vector`에 저장한 할 일을 파일에 쓰고, 다시 파일에서 읽어 출력합니다.
...
할 일이 여러 개라면 `vector<string>`을 사용합니다.
...
`ofstream`을 사용하면 파일에 내용을 쓸 수 있습니다.
...
프로그램을 실행하면 같은 폴더에 `todos.txt` 파일이 만들어집니다.

## 4단계: 파일에서 읽기

`ifstream`을 사용하면 파일 내용을 읽을 수 있습니다.
...
`getline`은 파일에서 한 줄을 읽어 `line`에 저장합니다. 읽을 줄이 없으면 반복문이 끝납니다.

## 5단계: 저장과 읽기 흐름 연결하기

Todo List 프로그램의 기본 흐름은 다음과 같습니다.

1. `vector`에 할 일을 저장합니다.
2. `ofstream`으로 파일에 씁니다.
3. `ifstream`으로 파일에서 다시 읽습니다.
4. 읽은 내용을 화면에 출력합니다.
...
`cin >> menu` 다음에 `getline`을 바로 쓰면 남아 있는 줄바꿈 문자 때문에 입력이 건너뛰어질 수 있습니다. 이 경우 `cin.ignore()`가 필요합니다.
...
- 범위 기반 `for`문으로 vector를 출력할 수 있다.
