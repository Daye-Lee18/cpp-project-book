...
- <mark>try</mark>: "여기서 예외가 발생할 수 있다"고 표시
- <mark>catch</mark>: 발생한 예외를 받아서 처리
- <mark>runtime_error&amp; e</mark>: 예외 객체를 참조로 받음 (복사 비용 절약)
...
- <mark>throw</mark>로 오류 상황을 알릴 수 있다.
- <mark>try-catch</mark>로 예외를 처리할 수 있다.
- <mark>e.what()</mark>으로 예외 메시지를 출력할 수 있다.
