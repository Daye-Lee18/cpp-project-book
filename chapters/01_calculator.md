...
C++ 프로그램은 보통 `main` 함수에서 시작합니다. 먼저 화면에 문장을 출력하는 가장 작은 프로그램을 작성해 봅니다.
...
- `a`와 `b`의 값을 바꾸면 출력 결과도 바뀝니다.
- `int`는 정수를 저장하는 자료형입니다.
- `10 / 3`의 결과가 `3.333...`이 아니라 `3`으로 나오는 이유는 `int`끼리 나누었기 때문입니다.

소수점 결과가 필요하면 `double`을 사용합니다.
...
매번 코드 안의 숫자를 고치는 계산기는 불편합니다. `cin`을 사용하면 실행 중에 사용자가 값을 입력할 수 있습니다.
...
`cin >> a >> b;`는 사용자가 입력한 첫 번째 값을 `a`에, 두 번째 값을 `b`에 저장합니다.

## 4단계: 연산자 입력받기

계산기가 덧셈만 할 수 있으면 아직 부족합니다. 사용자가 `+`, `-`, `*`, `/` 중 어떤 연산을 원하는지도 입력하게 만듭니다.
...
여기서 `a`에는 `10`, `op`에는 `'+'`, `b`에는 `3`이 저장됩니다.

## 5단계: 조건문으로 계산 나누기

입력된 연산자에 따라 다른 계산을 해야 합니다. 이때 `if` 문을 사용합니다.
...
- `%` 연산을 추가하세요. 단, `%`는 정수끼리만 사용할 수 있습니다.
- 계산을 한 번만 하지 말고 사용자가 `q`를 입력할 때까지 반복하도록 바꾸세요.
- 결과를 출력할 때 `Result: 13`처럼 앞에 설명을 붙이세요.
